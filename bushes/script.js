...
scr.onload = () => {
	...
	_tspan.innerText = TEXTS["text.bush"]
	title.append(_tspan)

	let content = ""
	
	const t_free = TEXTS["text.free"]
	
	for (let item of BUSHES) {
		let cls = 'class="item-name"'
		let n = TEXTS[item.n]
		if (!n) n = item.n
		...
		if (item.t) {
			details = `<span><i class="tle ${item.t}"></i><span>${item.a}</span></span>`
		} else {
			details = `<span>${t_free}</span>`
		}

		content += `<span>
			<span class='item-img'><img src="/bushes/${item.id}.webp" loading="lazy"></span>
			<span class='item-details'><span ${cls}>${n}</span><span class="item-info">${details}</span></span>
		</span>`
	}
